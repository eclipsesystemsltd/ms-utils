...
static char error_messages[27][60] = {
   " \n",                                               /* 0 */
...
void main(int argc,
	  char *argv[])
   {
   struct variant_ind variant_brackets[5];
...
//   char long_part_number[21];
   ...
   enum boolean six_digit_part_no = false;
   ...
   for (i=0; i<101; i++)	/* Clear input line */
      string_in[i] = ' ';
   ...
   while(fgets(string_in, 100, fptr_in) != NULL)
	 ...
	 if (string_in[position] == '*')
		...
		*	  (variant_level - 1) * 6], directive_string, 4); */
	       str_build_ltd(&string_out[variant_position +
				(variant_level - 1) * 3], directive_string, 4);
	       print_line = true;
	       /*print_line = false; */
               }
		...
		*	   (variant_level - 1) * 6], directive_string, 4); */
		  ...
		  variant_assy_level = variant_assy_number;
	       ...
	       print_line = false;
               ...
               if (variant_assy_defn_complete == true)
                  {
                  i = 1;
                  while (strncmp(variant_assy[i].label, directive_string, 3)
                                                                      != 0 &&
		 ...
		 /* following statement may over run array limit */
	       str_build_ltd(&string_out[quantity_position - 10],
						  directive_string, 10);
	    ...
	    if (error_type == 0)
		  ...
		  max_description_length = 59;
	       ...
	       if (check_ref_number(string_in,
				    ...
				    &position) != 0)
                  {
		  if ((chk_part_no = check_alpha_part_number(string_in,
						             string_out,
						             six_digit_part_no,
							     &position)) == 1)
		     {
		     ...
		     else if (assembly_level > 0)
			{
			   ...
			   if (quantity > 0)
				 ...
				 {
				 if (assembly[assembly_level].components_to_go
									   != 0)
				    error_type = 10;
				 assembly_level--;
				 }
			...
			string_out[quantity_position - 1] = '|';
		     ...
		     if (variant_level > 0)
                        {
                        display_variant(string_out,
                                        variant_brackets,
					variant_level);
                        }
		     if (variant_assy_level > 0)
                        {
                        display_variant_assy(string_out,
                                             variant_assy,
					     variant_assy_level);
                        }
                     if (assembly_position > 0)
			...
			if (check_alternative_alpha_part(string_in,
                                                         six_digit_part_no,
							 &position) != 1)
			   error_type = 22;
			}
			...
			if (check_optional_part(string_in,
						&position) != 1)
			   error_type = 24;
			}
		     if (long_part_number_position > 0)

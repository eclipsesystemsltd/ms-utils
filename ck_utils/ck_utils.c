...
//   char odd_part[30];
   ...
   while (fscanf(fptr_odd_parts,"%s %s", alpha_part_number, int_part_number) != EOF)
	 ...
	 if (six_digit_part_no == true &&
	    ...
	    while (int_part_number[j] != '\0')
	       {
	       string_out[j] = int_part_number[j];
	       j++;
	       }
	    }
	 else
   ...
   if (string_in[position] == '*')
	 ...
	 if (strcmp(alt_part_manufacturer, &alt_manufacturer_list[j][0]) == 0)
	    ...
	    while (string_in[position + i + k] != ' ' &&
	       ...
	       string_out[alt_part_number_posn + k - 1] = string_in[position + i + k];
		   if (string_in[position + i + k] == '_')	/* Check for '_' in part number 23/10/12 */
		      {
			     return(0);
		      }
	       k++;
...
/* Also check check that long part number is actually defined */
   ...
   if (isspace(string_in[*place]) == 0 &&
	 ...
	 {
/*	 *place = *place + 8;
	 str_build(&string_out[part_number_position], "see RH "); 15/6/12 */
	 str_build_ltd(&string_out[part_number_position], &string_in[*place], 7); /* 15/6/12 */
	 *place = *place + 8; /* 15/6/12 */
	 return(1);
	 }
      }   return(0);
...
int get_section(char section[max_lines][200],
		int file_no,
		int section_no,
   ...
   if (section_found == true)
      ...
      section_read = false;
      in_section = true;
      while (fgets(string_in, 200, fptr_file) != NULL &&
	     in_section == true)
	 {
	 if (is_blank_line(string_in) == 0)
	    {
...
//   char number[5];
   ...
   for (i = 1; i < title_posn; i++)      /* Go through each file looking for */
	 ...
	 /* printf("Opened file %s\n",temp_file); */
	 while (fgets(string_in, 200, fptr_list) != NULL)
	    {
	    if (is_blank_line(string_in) == 0)
	       {
	       k = 0;
...
int display_float_data(char string_out[],
   ...
   buffer = _fcvt(number,2,&dec,&sign);
   k = position;
   while (buffer[i] != '\0')
      i++;
   for (j=i-1; j>=0; j--)

                               /* pp_scan.c */
/*
PPX_SCAN.C

* As PP_SCAN.C but modified to handle parts lists with rows that don't start with a
reference number.

This program converts a file produced by a scan of a parts list into the
normal parts list format. It correctly positions reference number, part number,
description and quantity. Note that some editing of the scan file may be
necessary, e.g. to insert *title directives. Lines beginning with a '*' are
not processed.

The output is printed to the screen.

E.g. C:> PP_SCAN T140_78.SCN  where T140_78.SCN is a text file generated by
the scanning process.
*/

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h>
#include "../ck_utils/ck_types.h"
#include "../ck_utils/ck_utils.h"

void main(int argc,
	  char *argv[])
   {
   FILE *fptr_scan;
   char string_in[201];
   char string_out[101];
   char ref_string[20];
   char quantity[10];
   char alt_part_manufacturer[20];
   char alt_part_number[20];
   char alternative_part_number[40];
   char dummy_string[200];
   enum boolean first_line = true;
   enum boolean blank_line = false;
   enum boolean comment_line = false;
   enum boolean alternative_part = false;
   enum boolean quantity_found;
   int i,j,k,l;
   int desc_ref_point;
   int no_of_spaces;
   int position;
   int alt_part_number_position;
   int ref_number;

   if ((fptr_scan = fopen(argv[1],"r")) == NULL)
      {
      printf("Cannot open file %s \n",argv[1]);
      exit(0);
      }
   while(fgets(string_in, 200, fptr_scan) != NULL)
      {
      for (i=0; i<100; i++)
         string_out[i] = ' ';
      string_out[0] = '\0';
      i=0;
      blank_line = false;
      comment_line = false;
      alternative_part = false;
      quantity_found = false;
      position = 0;
      alt_part_number_position = 0;
      if (string_in[0] == '*')
         {
         printf("%s", string_in);
         comment_line = true;
         if (strncmp("*title", string_in, 6) == 0) {
            first_line = true;
            ref_number = 1;			// - PPX
            }
         }
      else
         {
         /* Remove leading spaces */
         while (string_in[i] == ' ' ||
                string_in[i] == '\t')
            i++;
         if (string_in[i] == '\0' ||
             string_in[i] == '\n')
            blank_line = true;
         }
      if (blank_line == false &&
          comment_line == false)
         {
         /* Look for alternative manufacturer part number */
	 k=1;
	 while (string_in[k] != '\n'&&
		string_in[k] != '\0')
	    {
	    if (string_in[k] == '*')
	       if (alt_part_number_position == 0)
                  {
		  position = k;
                  if (check_alt_part_number(string_in,
                                            alt_part_manufacturer,
                                            alt_part_number,
                                            dummy_string,
			     	            &position) == 1)
                     {
                     alternative_part = true;
                     build_alt_part_number(alternative_part_number,
                                           alt_part_manufacturer,
                                           alt_part_number);
                     for (l=0; l <= strlen(alternative_part_number); l++)
                        string_in[k + l] = ' ';   
                     }
                  }
	       else
		  {
		  printf("ERROR: ALT PART NUMBER\n");
		  break;
		  }
	    k++;
	    }

         if (string_in[i] != '\0' &&
             string_in[i] != '\n')	/* Redundant statement */
            {

            /* Insert reference number, covers case of lists of part
             * numbers which do not start with reference numbers - PPX */
            j=0;
            itoa(ref_number, ref_string, 10);
            while(ref_string[j]) {
               string_out[j] = ref_string[j];
               ++j;
               }
            ++ref_number;

            /* Get part number (2nd field) */
            j=3;
            no_of_spaces = 0;
            while (no_of_spaces < 2 &&
                   string_in[i] != '\0' &&
                   string_in[i] != '\n')
               {
               if (string_in[i] != ' ')
                  {
                  string_out[j] = string_in[i];
                  no_of_spaces = 0;
                  j++;
                  }
               else
                  no_of_spaces++;
               i++;
               }
            if (string_in[i] != '\0' &&
                string_in[i] != '\n' &&
                j < 11)
               {
               /* Remove spaces */
               while (string_in[i] == ' ' &&
                      string_in[i] != '\n' &&
                      string_in[i] != '\0')
                  i++;
               if (string_in[i] != '\n' &&
                   string_in[i] != '\0')
                  {
                  /* Get description (remaining fields) */
                  if (first_line == true)
                     {
                     strcpy(&string_out[11], &string_in[i]);
                     desc_ref_point = i;
                     first_line = false;
                     }
                  else
 /*                    strcpy(&string_out[11 + (i - desc_ref_point)], &string_in[i]);  - PPX */
                     /* Ignore description indentation */
                     strcpy(&string_out[11], &string_in[i]);
                  /* Remove multiple spaces from description */
                  /* Ignore description indentation */
                  i = 11;
                  while (string_out[i] == ' ')
                     i++;
                  i++;
                  while (string_out[i] != '\0' &&
                         string_out[i] != '\n')
                     {
                     if (string_out[i] == ' ' &&
                         string_out[i - 1] == ' ')
                        {
                        j = i;
                        while (string_out[j] == ' ')
                           j++;
                        strcpy(&string_out[i], &string_out[j]);
                        }
                     i++;
                     }
                  /* Remove trailing blanks */
                  i--;
                  while (isspace(string_out[i]) != 0)
                     {
                     i--;
                     }
                  string_out[i + 1] = '\n';
                  string_out[i + 2] = '\0';
                  /* Put $ in quantity field (last field) */
                  if (isdigit(string_out[i]) &&
                      isdigit(string_out[i - 1]) &&
                      string_out[i - 2] == ' ')
                     {
                     quantity[0] = '$';
                     strcpy(&quantity[1], &string_out[i - 1]);
                     quantity_found = true;
                     string_out[i - 1] = '\0';
                     }
                  else if (isdigit(string_out[i]) &&
                     string_out[i - 1] == ' ')
                     {
                     quantity[0] = '$';
                     strcpy(&quantity[1], &string_out[i]);
                     quantity_found = true;
                     string_out[i] = '\0';
                     }
                  if (alternative_part == true)
                     {
                     if (quantity_found != true)
                        {
                        string_out[strlen(string_out) - 1] = ' ';
                        strcat(string_out, alternative_part_number);
                        strcat(string_out, "\n");
                        }
                     else
                        strcat(string_out, alternative_part_number);
                     }
                  if (quantity_found == true)
                     {
                     if (alternative_part == true)
                        strcat(string_out, " ");
                     strcat(string_out, quantity);
                     }
                  }
               }
            else
               {
               string_out[j] = '\n';
               string_out[j + 1] = '\0';
               }
            }
         else
            {
            string_out[j] = '\n';
            string_out[j + 1] = '\0';
            }
         printf("%s", string_out);
         }
      }
   }  /* main() */